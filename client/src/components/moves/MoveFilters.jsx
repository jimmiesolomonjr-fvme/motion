...
export default function MoveFilters({ filters, onFilterChange }) {
  ...
  return (
    <div className="mb-2">
      {/* Single scrollable row: time chips | category chips | sort dropdown */}
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
        ...
        <div className="w-px h-4 bg-dark-50 flex-shrink-0" />
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap bg-dark-50 text-gray-400 hover:text-white transition-colors"
          >
            {currentSort.label}
            <ChevronDown size={12} className={`transition-transform ${showSort ? 'rotate-180' : ''}`} />
          </button>
          {showSort && (
            <div className="absolute right-0 top-full mt-1 bg-dark-100 border border-dark-50 rounded-xl shadow-xl z-50 min-w-[120px]">
              {SORT_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => setSort(opt.value)}
                  className={`w-full text-left px-3 py-2 text-xs transition-colors first:rounded-t-xl last:rounded-b-xl ${
